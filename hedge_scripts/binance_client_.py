...
class BinanceClient(object):
    ...
    ### FUNCTIONS
    def minutes_of_new_data(self, kline_size, data, source):
        if len(data) > 0:
            old = parser.parse(data["timestamp"].iloc[-1])
        elif source == "binance":
            old = datetime.strptime(self.initial_date, '%d %b %Y')
        if source == "binance":
            new = pd.to_datetime(self.client.get_klines(symbol=self.symbol, interval=kline_size)[-1][0], unit='ms')
        return old, new
    ...
    def get_all_binance(self, save=False):
        ...
        oldest_point, newest_point = self.minutes_of_new_data(self.freq, data_df, source="binance")
        delta_min = (newest_point - oldest_point).total_seconds() / 60
        available_data = math.ceil(delta_min / binsizes[self.freq])
        ...
        klines = self.client.get_historical_klines(self.symbol, self.freq,
