...
class BinanceClient(object):
    ...
    ### FUNCTIONS
    def minutes_of_new_data(self, symbol, kline_size, initial_date, data, source):
        if len(data) > 0:
        ...
        if source == "binance":
            new = pd.to_datetime(
                self.client.get_klines(symbol=symbol, interval=kline_size)[-1][0],
                unit="ms",
            )
        return old, new

    def get_all_binance(self, symbol, freq, initial_date, save=False):
        binsizes = {
            "1m": 1,
            "5m": 5,
            "10m": 10,
        ...
        filename = (
            "/home/agustin/Git-Repos/HedgingScripts/files/%s-%s-data_since_%s.csv"
            % (symbol, freq, initial_date)
        )
        data_df = pd.DataFrame()
        oldest_point, newest_point = self.minutes_of_new_data(
            symbol, freq, initial_date, data_df, source="binance"
        )
